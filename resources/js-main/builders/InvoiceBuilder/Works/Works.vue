<template>
  <div>
    <h3 class="contract__gray-box-title">
      List of Services
      <v-tooltip right>
        <img class="contract-draft__tooltip" slot="activator" :src="'/img/icon/info-icon_gray.svg'">
        <span>
          Used to provide an overview of required; about service costs relating to the cost of employees or sub-trades
        </span>
      </v-tooltip>
    </h3>
    ...
    <div class="list-works-invoice__list">
      ...
      <div class="list-works-invoice__elment" v-for="(work, index) in getList" :key="index">
        <work :readOnly="readOnly" :index="index" :work="work" @removeWork="removeWork" />
      </div>
