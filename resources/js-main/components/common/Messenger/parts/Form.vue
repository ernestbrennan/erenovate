...
<script>
  ...
  import {messengerTemplate} from '@/components/plugins/dropzoneTemplates'
  ...
  export default {
    ...
    computed: {
      ...
      text_input_placeholder() {
        if (this.message.files.length) {
          return 'Add a comment to the file'
        }
        return 'Send Message'
      },
      state_send_button() {
    ...
    methods: {
      ...
      sendMessage(params) {
        this.$store.dispatch('sendMessage', params).then(response => {
