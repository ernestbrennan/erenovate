<template>
  <div class="contract-timeline-comp scrollbar component-scroll" :style="{height: componetConteinerHeight}">
    <div class="contract-timeline">
      <ul class="contract-timeline__list" data-v-step="ilho-2" v-if="timeline">
        ...
        <timeline-elem
          data-v-step="ci-9" v-if="$route.name === 'list-invoice'"
        ...
        <template v-for="(item, index) in timeline.milestones">
          ...
          <timeline-elem
            v-if="timeline.invoice && timeline.invoice.milestone_id === item.id"
          ...
          <timeline-elem
            data-v-step="ci-9" v-if="$route.name === 'create-invoice' && item.status === 'in_progress'"
            :elem="{status: '', title: getTitle()}"
            :to="{name: $route.name, params: $route.params}"
          />
