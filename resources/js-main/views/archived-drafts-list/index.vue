<template>
  <div class="contracts-comp">
    <div :class="{ slide: slideSearchContracts}" class="contracts-comp__header project-list__contractor">
      <div class="cotracts-comp__search-box">
        <img src="/img/icon/search_gray.svg" alt="search" class="cotracts-comp__search-icon">
        <input
          placeholder="Search a Project by title, description or Pro name"
        ...
        <template v-if="query_search.length > 0">
          <v-tooltip bottom>
            <img
              ...
              class="cotracts-comp__clean-search"
              src="/img/icon/close__md_gray.svg"
              alt="clear_search"
            >
      ...
      <v-btn fab depressed class="cotracts-comp__submit-btn" @click="getArchivedDraftsList">
        <img src="/img/icon/arrow_gray.svg">
      </v-btn>

      <router-link class="main-btn main-btn_full-blue" to="/invite-ho" tag="button">START A PROJECT</router-link>
    </div>
    <div class="contracts-comp__body scrollbar">
      <template v-if="list.length">
        <div
          class="contract-list"
          v-for="(archived_draft, index) in list"
          :key="index"
          @click="toArchivedDraft(archived_draft.id, true)"
        >
          <div class="contract-list__img"><img :src="user.photo"></div>
          ...
          <button
            class="main-btn main-btn_border-red"
            @click="delete_id = archived_draft.id; dialog_delete_draft = true;"
          >
            DELETE
          </button>
        ...
        <div class="contract-list__no-contracts">
          <div class="contract-list__no-contracts-inner">
            <template v-if="!query_search">
              <img
                class="common-img_center common-img_m-b-40-35"
                src="/img/contracts-list_empty.svg"
                alt="empty"
              >
              <h4 class="common-h4">No Project Drafts found.</h4>

              <p class="common-p">Let’s create Your 1<sup>st</sup> Project</p>
              <p class="common-p">
            ...
            <template v-else>
              ...
              <button class="common-btn common-btn_blue" @click="query_search = ''; getArchivedDraftsList()">
                CLEAR SEARCH
              </button>
            </template>
          </div>
    ...
    <delete-draft-dialog v-model="dialog_delete_draft" :loading="loading" @delete="handleDelete"/>
  </div>
</template>
...
<script>
  ...
  import {listArchivedDraft, deleteArchivedDraft} from '@/api/archived-draft'
  ...
  export default {
    ...
    methods: {
      handleDelete() {
        deleteArchivedDraft(this.delete_id).then(response => {
          this.list.splice(this.list.findIndex((item, index) => item.id === this.delete_id), 1);
      ...
      getArchivedDraftsList() {
        ...
        listArchivedDraft(this.query_search).then(response =>  {
