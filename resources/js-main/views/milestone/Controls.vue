    ...
    <div ref="contractHeader" class="contract-comp__header">
      <div class="g-flex g-flex_row g-flex_wrap margin-10 g-flex_vertical-center">
        ...
        <template v-else-if="milestone.edited.is_edited">
          <div
            class="contract-header__contract-label"
            :class="{'hidden-md-and-down': !is_edited}"
            v-if="is_edited_by_current_user"
          >
          ...
          <button
            data-v-step="mls-2"
            class="main-btn main-btn_border-blue-deep"
        ...
        <template v-else-if="this.milestone.milestone_content.status === 'active'">
          ...
          <template v-if="read_only_milestone">
            <v-spacer class="hidden-md-and-down"></v-spacer>
            <button
              class="mls-5 main-btn_border-green main-btn"
              v-if="can_complete"
            ...
            <v-spacer class="hidden-md-and-up"></v-spacer>
            <button class="main-btn main-btn_border-blue-deep" @click="$store.state.read_only_milestone = false">
              {{propose_text}}
            </button>
          </template>
...
<script>
  ...
  import {proposalConfDialog, acceptEditDialog, rejectEditDialog} from '@/components/dialog/milestoneEditing';

  import InitTour from '@/plugins/tour/init'
  import MilestoneTour from '@/plugins/tour/milestone'
  import validate_milestone from '@/components/mixins/validator/milestone'
  import DialogWithComment from '@/components/common/elements/DialogWithComment'
  import {ChildMixin} from '@/components/common/Mixins/glValidate/mixins.js'

  import CompleteDialog from './dialos/CompleteDialog'
  import EditDialog from './dialos/EditDialog'

  import {editMilestone, acceptMilestone, rejectMilestone} from '@/api/milestone';
  ...
  export default {
    ...
    methods: {
      ...
      acceptMilestone() {
        ...
        acceptMilestone(this.milestone, this.dialog_comment).then(response => {
      ...
      rejectMilestone() {
        ...
        rejectMilestone(this.milestone, this.dialog_comment).then(response => {
          if (this.$route.query.status) {
            this.$router.replace({'query': null});
          }
          this.isDialogWithComOpen = false
      ...
      editMilestone(comment) {
        ...
        editMilestone(this.milestone, comment).then(response => {
