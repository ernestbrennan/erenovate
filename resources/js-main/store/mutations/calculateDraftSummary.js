export function updateDraftSummary(state, type) {
        ...
        case 'material_cost':
            ...
            state.contract_draft.milestones.map((item) => {
                ...
                item.milestone_content.materials.forEach((material) => {
                    if (item.milestone_content.material_supply_side === 'contractor'  && item.milestone_content.materials_provide_on === 'contract') {
                        material_cost += g_parsePrice(material.price) * material.quantity;
                    }
                })
            });
