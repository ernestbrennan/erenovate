...
import {updateDraftInfoUser} from "@/api/user";
...
export default {
  ...
  updateUserInfo(context, current_user_info) {
    return new Promise((resolve) => {
      ...
      updateDraftInfoUser(current_user_info).then(response => {
        context.commit('updateContractProp', {prop: 'current_user_info', value: response.data.response});
