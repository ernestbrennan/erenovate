<template>
    <div ref="contractHeader" class="contract-comp__header contract-comp__header_summery">
        <div class="g-flex g-flex_row g-flex_wrap margin-10 g-flex_vertical-center" :class="{'g-flex_summary-page': !needSpace}">
            <div  class="contract-header__title" :class="{'hidden-sm-and-down': summary.contract.status === 'finished'}">
                Project "{{ project_name | substring_title}}" Summary
            </div>
            <div  class="sum-tour-1 contract-header__contract-label"  v-if="summary.contract.status === 'finished'">
                Project Completed Approval Pending
            </div>
            <!--<v-spacer></v-spacer>-->
            <div class="sum-tour-2" v-if="!needConfirmComplete"></div>
            <button class="sum-tour-2 main-btn main-btn_border-blue-deep hidden-sm-and-down"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            needSpace(){
                return this.summary.contract.status === 'finished' && this.user.role === 'contractor'
            },
